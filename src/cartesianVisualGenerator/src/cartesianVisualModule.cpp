...
/**
 * @file cartesianVisualModule.cpp
 * @brief Implementation of the cartesianVisualModule (see header file).
 */
...
bool cartesianVisualModule::configure(yarp::os::ResourceFinder &rf) {
    /* Process all parameters from both command-line and .ini file */

    /* get the module name which will form the stem of all module port names */
    moduleName            = rf.check("name", 
                           Value("/cartesianAudio2Visual"), 
    ...
    */
    ...
    rThread = new cartesianVisualThread(robotName, configFile);
...
bool cartesianVisualModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool cartesianVisualModule::close() {
    handlerPort.close();
    /* stop the thread */
...
bool cartesianVisualModule::respond(const Bottle& command, Bottle& reply) 
{
    bool ok = false;
...
bool cartesianVisualModule::updateModule()
{
    return true;
}

double cartesianVisualModule::getPeriod()
{
